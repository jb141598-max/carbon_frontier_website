...
function normalizeUpdate(update, index = 0) {
  ...
  const requestedPublishedAt = String(update?.publishedAt || "").trim();
  const publishedTimestamp = Date.parse(requestedPublishedAt);
  ...
  try {
    ...
    if (!viewer.canManageSettings) {
      return {
        ok: false,
        status: 403,
        message: "Only Carbon Frontier owners and admins can manage updates.",
      };
    }
